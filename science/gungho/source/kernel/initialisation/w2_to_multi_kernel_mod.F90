...
module w2_to_multi_kernel_mod
  ...
  !> Psy layer.
  !>
  type, public, extends(kernel_type) :: w2_to_multi_kernel_type
    private
  ...
  contains
    procedure, nopass :: w2_to_multi_code
  end type

  !---------------------------------------------------------------------------
  ! Contained functions/subroutines
  !---------------------------------------------------------------------------
  public :: w2_to_multi_code

contains

!> @brief The subroutine which is called directly by the psy layer
!! @param[in] nlayers     Integer the number of layers
!! @param[in,out] multi_u Multi-data wind field
!! @param[in] w2_u        W2 wind field
...
subroutine w2_to_multi_code( nlayers,                       &
                             multi_u,                       &
                             w2_u,                          &
  ...
  real(kind=r_def), dimension(undf_mult), intent(inout) :: multi_u
  real(kind=r_def), dimension(undf_w2),  intent(in)    :: w2_u
  ...
  do k = 0, nlayers-1
    do df = 1, 6
      ...
      multi_u(map_mult(1) + (df-1)*nlayers + k) = w2_u(map_w2(df) + k )

    end do
  end do

end subroutine w2_to_multi_code

end module w2_to_multi_kernel_mod
